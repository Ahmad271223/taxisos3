...
// im Stil der Provisionsrechnung (lib/pdf.ts).

import { PDFDocument, StandardFonts, rgb, type PDFFont, type PDFPage } from "pdf-lib";
import type { InstitutionStatement } from "@/lib/institutionInvoice";
...
export async function institutionStatementPdf(s: InstitutionStatement): Promise<Uint8Array> {
  ...
  text("EINRICHTUNG", M, y, 8, bold, MUTED);
  y -= 15;
  text(s.institution?.name ?? "Einrichtung", M, y, 12, bold);
  y -= 14;
  text(TYPE_LABEL[s.institution?.type ?? ""] ?? (s.institution?.type ?? ""), M, y, 10, font, MUTED);
  if (s.institution?.address) { y -= 13; text(s.institution.address, M, y, 10, font, MUTED); }

  right("Fahrten gesamt", RIGHT, y + 27, 8, bold, MUTED);
  right(String(s.rides), RIGHT, y + 13, 13, bold);
  right(`davon abgeschlossen: ${s.completed}`, RIGHT, y, 9, font, MUTED);

  y -= 28;
  text(`Krankenfahrten im Zeitraum ${s.periodLabel}. Abgeschlossene Fahrten sind abrechenbar; geplante Fahrten nachrichtlich.`, M, y, 9.5, font, INK);
  y -= 22;

  // ---- Tabelle ----
  const COL = { date: M, patient: M + 62, route: M + 190, status: 430, amount: RIGHT };
  const drawHeader = () => {
  ...
  for (const l of s.lines) {
    ...
    text(clip(l.route, COL.status - COL.route - 8), COL.route, y, 9, font);
  ...
  y -= 4; hline(y); y -= 20;
  const labelX = 330;
  text("Geplant (nachrichtlich)", labelX, y, 10, font, MUTED);
  right(eur(s.totalPlanned), RIGHT, y, 10, font, MUTED);
  y -= 8; hline(y); y -= 18;
  text("Abrechenbar gesamt", labelX, y, 12, bold);
  right(eur(s.totalBillable), RIGHT, y, 12, bold);

  // ---- Fuss ----
  let fy = 84;
  hline(fy + 12);
  text("Leistung: Krankenbeförderung. Abrechnung der abgeschlossenen Fahrten des Zeitraums.", M, fy, 8.5, font, MUTED);
  fy -= 12;
